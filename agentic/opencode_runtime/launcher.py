"""Launch MCP uvicorn → wait /health → ``opencode run …`` + broker SSE ingest."""
...
from .opencode_json import write_opencode_config

log = logging.getLogger("opencode-runtime-launcher")
...
def _build_opencode_argv(settings: cfg_module.OpenCodeSettings) -> List[str]:
    if settings.dev_command_override.strip():
        return shlex.split(settings.dev_command_override)
    cmd: List[str] = [settings.opencode_bin, "run"]
    if settings.opencode_model.strip():
        cmd.extend(["--model", settings.opencode_model])
    extra = shlex.split(settings.opencode_run_extra) if settings.opencode_run_extra else []
    cmd.extend(extra)
    cmd.append(settings.session_prompt)
    return cmd


def _build_opencode_env(settings: cfg_module.OpenCodeSettings) -> dict:
    env = dict(os.environ)
    ...
    xdg_cfg = Path(settings.home_dir) / ".config"
    env.setdefault("XDG_CONFIG_HOME", str(xdg_cfg))
    env["PYTHONUNBUFFERED"] = "1"
    rpc = cfg_module.mcp_rpc_url(settings)
    env["CHAT_AI_MCP_RPC_URL"] = rpc
    cfg_path = xdg_cfg / "opencode" / "opencode.json"
...
async def run(settings: Optional[cfg_module.OpenCodeSettings] = None) -> int:
    settings = settings or cfg_module.get_settings()
    _configure_logging(settings.log_level)
    log.info(
        "opencode.launcher.start",
        extra={
            "version": __version__,
    ...
    argv = _build_opencode_argv(settings)
    proc = await asyncio.create_subprocess_exec(
        *argv,
        env=_build_opencode_env(settings),
        cwd=settings.home_dir,
        stdout=asyncio.subprocess.PIPE,
        stderr=asyncio.subprocess.STDOUT,
    )

    async def oc_lines() -> AsyncIterator[str]:
    ...
    forward_task = asyncio.create_task(
        ...
        name="opencode-sse-forward",
    )

    rc = 1
    try:
        rc_val = await asyncio.wait_for(
            proc.wait(), timeout=settings.opencode_max_runtime_s,
        )
        rc = int(rc_val) if rc_val is not None else 1
    except asyncio.TimeoutError:
        log.error(
            "opencode_runtime_cap",
            extra={"s": settings.opencode_max_runtime_s},
        )
        await _terminate(proc, "opencode")
        rc = 124
    finally:
        try:
    ...
    log.info("opencode.launcher.done", extra={"opencode_rc": rc})
    return rc
...
def main(argv: Optional[List[str]] = None) -> int:
    p = argparse.ArgumentParser(prog="opencode-runtime-launcher")
